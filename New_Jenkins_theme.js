...
// EOC
	var addUncheckButton = function(parameterBody) {
		...
		btn.setButtonState = function() {
			...
			//btn.textContent = (btn.checkboxesChecked) ? 'ÃƒÂ¢Ã‹Å“ÃƒÂ¢Ã‹Å“' : 'ÃƒÂ¢Ã‹Å“Ã¢â‚¬ËœÃƒÂ¢Ã‹Å“Ã¢â‚¬Ëœ';
		}
		...
		$('#j_username').each(function(){
			...
			$.on(this, 'change', function() {
				...
				if (login.length && login[0].toLowerCase() === login[0]) {
					alert('Login MUSI byÄ‡ wpisany z duÅ¼ej litery!'
						+'\n\nInaczej nie bÄ™dziesz mieÄ‡ normalnych uprawnieÅ„.'
						+'\n\nPrzykÅ‚ad: jÃ³zefk - Åºle, JÃ³zefK - dobrze.'
					);
				}
			});

			var el = document.createElement('span');
			el.textContent = ' Login MUSI byÄ‡ wpisany z duÅ¼ej litery (jÃ³zefk - Åºle, JÃ³zefK - dobrze).';
